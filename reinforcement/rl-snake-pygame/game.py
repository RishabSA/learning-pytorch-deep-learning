...
BLOCK_SIZE = 20
SPEED = 40  # Use 20 for humans, 40 for AI
...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        if self.is_collision() or self.frame_iteration > 100 * len(
            self.snake
        ):  # If the agent does not accumulate any reward for a while or it hits a collision, end the game
            game_over = True
            reward = -10
    ...
    def is_collision(self, point=None):
        ...
        ):  # Ignore the head because head is always hitting itself
            return True
